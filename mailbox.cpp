...
unsigned int mailbox_setup(unsigned char Channel)
{
    ...
    if ((!MailboxHandle) || (!MailboxPacket)) { debug("[MBOX]: Lock-up in Sector 7G, Sir"); return MAILBOX_FAILURE; }
    ...
    mmio_write(MailboxHandle, 0, 0x0);
    mmio_write(MailboxHandle, 1, 0x0);
    mmio_write(MailboxHandle, 2, 0x0);
    mmio_write(MailboxHandle, 3, 0x0);
    mmio_write(MailboxHandle, 4, 0x0);
    mmio_write(MailboxHandle, 5, 0x0);
    mmio_write(MailboxHandle, 6, 0x0);
    mmio_write(MailboxHandle, 7, 0x0);
